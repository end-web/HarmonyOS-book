...
本文面向希望为「简听」编写书源的作者，依据当前项目的导入解析器、请求分析器、规则执行器和页面调用链整理。核对日期：2026-09-16。
...
| `enabledExplore` | boolean | `false` | 是否启用发现 |
...
| `updateTime` | 更新时间规则 | 会被导入；当前通用搜索结果映射未使用，勿依赖其展示 |
...
字段多值通常以换行合并；URL 字段通常选第一个可解析地址。列表字段则保持列表语义。
...
首页只选择已启用的 **音频源**。通用首页预览前 4 个可用分类，每个分类最多展示 12 本音频书；分类里的动作按钮不作为首页分类。用户需要在书源页选择首页源，不会自动汇总所有导入源。

小说源可以提供发现入口，但不会因此成为听书首页源。听友导入源的首页保留专用协议适配。
...
| 光遇 / 书山 / 听友 / Talebook | 已有特定导入源身份与原生协议适配，不是可在 JSON 中任意声明的新协议插件 |
