...
当前版本：`0.1.14`，最低 API 20 / 目标 API 24，`com.huan.listenbook`。版本以 [AppScope/app.json5](AppScope/app.json5) 为准。

## 下载与更新

[最新版本与更新说明](https://github.com/end-web/HarmonyOS-book/releases/latest) · [下载简听 v0.1.14 安装包](https://github.com/end-web/HarmonyOS-book/releases/download/v0.1.14/JianTing-v0.1.14.hap)
...
```bash
hdc install JianTing-v0.1.14.hap
```

`v0.1.14` 支持在书源管理中选择首页听书源，“精品推荐”置首，其余分类按来源顺序展示；修复累计收听时长、折合本数等统计的持久化与加载问题，同时包含小说仿真翻页及书源页面安全区优化。包名保持 `com.huan.listenbook`，沿用已有书架、书源与记录。
...
- **首页浏览**：在书源管理中选择已启用的听书源，推荐与分类跟随选择；“精品推荐”置首，其余分类按来源顺序展示，支持骨架屏、下拉刷新和板块“更多”。
...
上传前运行 `python scripts/verify-release-package.py <APP或HAP路径> --version 0.1.14 --version-code 1000014`，核验 SDK 正式版标记、目标 API 24、最低 API 20、版本和 1024×1024 分层图标；签名有效性另用 SDK 的 `hap-sign-tool verify-app` 检查。
