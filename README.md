...
当前版本：`0.1.15`，最低 API 20 / 目标 API 24，`com.huan.listenbook`。版本以 [AppScope/app.json5](AppScope/app.json5) 为准。

`v0.1.15` 增加首页源快捷切换与主题色下拉菜单、固定在首位的分类总览和分类快速跳转，默认选中精品推荐；修复听友分类地址兼容与翻页，新增启动自动播放设置，优化“我的”菜单页面转场，默认预加载调整为 1 章。

## 下载与更新

[最新版本与更新说明](https://github.com/end-web/HarmonyOS-book/releases/latest) · [下载简听 v0.1.15 安装包](https://github.com/end-web/HarmonyOS-book/releases/download/v0.1.15/JianTing-v0.1.15.hap)
...
```bash
hdc install JianTing-v0.1.15.hap
```
...
上传前运行 `python scripts/verify-release-package.py <APP或HAP路径> --version 0.1.15 --version-code 1000015`，核验 SDK 正式版标记、目标 API 24、最低 API 20、版本和 1024×1024 分层图标；签名有效性另用 SDK 的 `hap-sign-tool verify-app` 检查。
