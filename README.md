...
- **书源管理**：本地 JSON、粘贴 JSON、HTTP(S) 地址导入、扫描二维码导入；名称/地址/分组检索、测试状态展示、批量启停与删除、单源测试和批量搜索测试。
...
1. 从”我的 → 书源”打开管理页，通过标题栏加号导入本地 JSON、粘贴规则、填写 HTTP(S) 地址或扫描二维码。
2. 扫描二维码支持：扫描书源分享二维码、包含下载链接的二维码，或从相册选择已保存的二维码图片。
3. 根据来源要求登录主账号或网站账号。单源测试在有结果时继续验证详情、目录和正文/音频；批量测试只检查搜索可用性。
