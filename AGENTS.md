...
- **SDK**: HarmonyOS 6.0+; `targetSdkVersion = 6.1.1(24)`, `compatibleSdkVersion = 6.0.0(20)` for both App products and the QuickJS HAR; `bundleName: com.huan.listenbook`. Build with a verified Release SDK; Beta/Canary artifacts must not be published. Use `PlatformCompat` for newer APIs and lazy imports for newer system modules.
