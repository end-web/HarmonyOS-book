...
- 首页分类切换独立过渡轮播占位、淡入内容并平滑定位标签；轮播节点保持完整尺寸，用单段 650ms 原生 Animator 同步标题栏配置高度和内容占位，连续收起/展开，快速反向切换从当前进度衔接。总览卡片进入可见区域后依次浮现，首屏交错 45ms，后续按行限制延迟。轮播不声明固定索引，可见性引起的自动播放暂停/恢复保留当前页，数据重置时才通过控制器回到首张；吸顶前最后 64vp 连同阴影淡出，搜索框在自身滚动行程结束后隐藏，避免封面底边和搜索框停留在透明状态栏内。首次加载骨架与真实轮播共用几何计算，推荐骨架与真实内容保持标题、书籍行和页面间距一致。
